...
// Revisit: the order that caused it is voided (both streams for that order); the revisit visit earns 0.
// Five KWD 5 bonuses in profitable months; ties SPLIT equally; zero-revisit splits among all qualifying trucks.
function computeIncentives(jobs, refDate, fixedExpenses = 12500) {
  ...
  const rows = trucks.map(t => {
    ...
    return {
      ...
      upsellPay: Math.round(ups * INCENT.upsellRate * 1000) / 1000, // always paid
  ...
  // five bonuses — profitable months only; ties split; zero-revisit splits among qualifiers
  const cats = [
    ["bOrders", (r) => r.orders, "Highest orders"],
    ["bReviews", (r) => r.avgReview || 0, "Highest reviews"],
    ["bRevenue", (r) => r.revenue, "Highest revenue"],
    ["bProfit", (r) => r.profit, "Highest profit"],
  ];
  ...
  rows.forEach(r => {
    r.payout = Math.round(((r.baseUnlocked ? r.basePot : 0) + r.upsellPay + r.bonusTotal) * 1000) / 1000;
  });
      ...
      <div className="card-body" style={{ padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", margin: "10px 0", background: profitGate ? "#E8F4EC" : "#FEF2F2", border: `1px solid ${profitGate ? "#BFDFC9" : "#FECACA"}`, borderRadius: 10, padding: "8px 12px" }}>
          ...
          <span style={{ fontSize: 11.5, color: "var(--muted)", display: "flex", gap: 5, alignItems: "center" }}>
            ...
            KWD · gates base incentive + bonuses; upsells always pay
          </span>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="rep-table" style={{ width: "100%", fontSize: 12.5 }}>
            <thead><tr>
              <th style={{ textAlign: "left" }}>Truck</th><th>Orders</th><th>vs target</th><th>Base (gated)</th><th>Upsells ×1.000</th><th>Voided</th>
        ...
        <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 8 }}>Payout/person = unlocked base + upsells (always) + bonus shares. Bonus winners provisional until month end; ties split.</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ marginTop: 10, textAlign: "right", fontSize: 13, fontWeight: 800 }}>
          This month if it ended today: <span style={{ color: "var(--success)" }}>KWD {(me.payout || 0).toFixed(3)}</span> / person
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 16 }}>
        ...
        <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 8 }}>Every order = 0.250 for each of you once unlocked. Every upsell = 1.000 cash, guaranteed. Quality protects it all — one revisit voids that order.</div>
      </div>
    </div>
  );
}
