...
function openServiceHistoryPDF(car, customer, jobs, mode) {
  ...
  const entries = carJobs.map(j => {
    ...
    return `<div class="entry">
      ...
      <div class="body-col"><div class="svc-line"><span class="svc-type">${esc(j.service_type || "Service")}</span>${km ? `<span class="svc-km mono">${esc(km)}</span>` : ""}</div>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}html{-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  ...
  <div class="page" id="pg">
    <div class="masthead">
      ...
      <div class="doc-id"><div class="doc-title">Vehicle Service History</div><div class="issued mono">Issued ${new Date().toLocaleDateString("en-GB")}</div></div>
    </div>
    <div class="identity">
      <div class="field"><div class="label">Customer</div><div class="value">${esc(customer.name || "")}</div></div>
      <div class="field"><div class="label">Vehicle</div><div class="value">${esc(car.brand)} ${esc(car.model)} ${esc(car.year || "")}</div></div>
      <div class="field"><div class="label">VIN / Plate</div>${car.plate ? `<span class="plate mono">${esc(car.plate)}</span>` : `<div class="value">—</div>`}</div>
    </div>
    <div class="summary">
      <div class="stat"><div class="num">${carJobs.length}</div><div class="label">Recorded visits</div></div>
      <div class="stat"><div class="num"><em>${esc(latest)}</em></div><div class="label">Latest mileage</div></div>
      <div class="stat"><div class="num" style="font-size:14px;padding-top:4px">${esc(range)}</div><div class="label">Service period</div></div>
    </div>
    <div class="log-heading">Service Log — Most Recent Last</div>
